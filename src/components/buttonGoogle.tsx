...
export default function AuthButton() {
    ...
    try {
      ...
      const googleAuthUrl =
        ...
        `redirect_uri=${encodeURIComponent(
          `${process.env.NEXT_PUBLIC_BASE_URL}/api/googleLogin`
        )}&` +
        `response_type=code&` +
        `scope=email profile&` +
        `access_type=offline&` +
  ...
  return (
    <button
      type="button"
      onClick={handleLogin}
      className="w-full flex items-center justify-center gap-3 bg-white text-black border border-black py-3 px-4 rounded-full font-normal hover:bg-gray-50 transition-colors focus:outline-none focus:border-gray-800"
    >

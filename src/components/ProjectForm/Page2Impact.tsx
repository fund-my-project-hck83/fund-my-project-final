...
export default function Page2Impact({
  formData,
  updateFormData,
  errors,
}: Page2Props) {
  ...
  const removeImpactMetric = (index: number) => {
    if (formData.impactMetrics.length > 1) {
      const newImpactMetrics = formData.impactMetrics.filter(
        (_, i) => i !== index
      );
      updateFormData({ impactMetrics: newImpactMetrics });
    }
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        {formData.impactMetrics.map((metric, index) => (
          <div
            key={index}
            className="relative grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 p-4 border border-gray-200 rounded-lg bg-gray-50"
          >
            ...
            <div>
              ...
              <input
                ...
                value={metric.number}
                onChange={(e) =>
                  handleImpactChange(index, "number", e.target.value)
                }
              ...
              />
              {errors[`impactMetrics.${index}.number`] && (
                <p className="mt-2 text-sm text-red-600 font-normal">
                  {errors[`impactMetrics.${index}.number`]}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleImpactChange(index, "description", e.target.value)
                }
                className={`w-full px-4 py-3 border rounded-full focus:outline-none focus:border-gray-800 transition-colors text-black bg-white placeholder-gray-500 font-normal ${
                ...
                placeholder="orang dapat di-reach"
              />
              {errors[`impactMetrics.${index}.description`] && (
                <p className="mt-2 text-sm text-red-600 font-normal">
                  {errors[`impactMetrics.${index}.description`]}
          ...
          formData.impactMetrics.length === 1 && (
            <div className="text-center">
              <p className="text-sm text-gray-500 font-normal">
                Kamu bisa menambahkan metrics sampai 5
              </p>
            </div>
          )}
      </div>
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            <h3 className="text-sm font-medium text-black">
              Tips untuk mengisi Impact Metric
            </h3>
            <div className="mt-2 text-sm text-gray-600 font-normal">
              <p>Buat metric impact yang dapat diukur, contohnya:</p>
              <ul className="list-disc list-inside mt-2 space-y-1">
                <li>&quot;200 pengunjung event dalam sehari&quot;</li>
                <li>&quot;500 orang mendapatkan benefit&quot;</li>
                <li>&quot;1000 produk UMKM terjual&quot;</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        <div>
          ...
          <p className="text-sm text-gray-600 font-normal mt-2">
            Tambah image yang menarik perhatian donor potensial
          </p>
        </div>
